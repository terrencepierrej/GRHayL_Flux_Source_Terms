...
extern "C" void IllinoisGRMHD_driver_evaluate_MHD_rhs(CCTK_ARGUMENTS) {
  ...
  if( sizeof(CCTK_REAL) < 8 ) printf("Error: IllinoisGRMHD assumes that CCTK_REAL is a double precision number. Setting otherwise will likely cause havoc with the conserv_to_prims solver.");

  if(cctk_nghostzones[0]<3 || cctk_nghostzones[1]<3 || cctk_nghostzones[2]<3) { printf("ERROR. Need at least 3 ghostzones for IllinoisGRMHD evolutions."); }

 ...
 */
void calculate_characteristic_speed_1th_direction(const rhss_paramstruct *restrict rhss_params, const prims_struct *restrict reconstructed_prims, const metric_quantities_struct *restrict metric_face_quantities, conservative_fluxes_struct *restrict conservative_fluxes) {
...
const double h_faceDD22 = metric_face_quantities->h_faceDD22;
  ...
  const double tmp_53 = -tmp_1*vet_faceU0*(2*tmp_45 + 2*tmp_47) + 2*tmp_50*u4_lU0*u4_lU1;
  const double tmp_54 = (1.0/2.0)*tmp_52*tmp_53;
  const double tmp_57 = -tmp_1*((vet_faceU0)*(vet_faceU0)) + (-((h_faceDD12)*(h_faceDD12))*tmp_33 + tmp_13*tmp_33*tmp_8)/(-((h_faceDD01)*(h_faceDD01))*tmp_28 + 2*h_faceDD01*h_faceDD02*h_faceDD12*tmp_20 - ((h_faceDD02)*(h_faceDD02))*tmp_26 - ((h_faceDD12)*(h_faceDD12))*tmp_21 + tmp_13*tmp_20*tmp_3*tmp_8);
  const double tmp_58 = (4*tmp_49 + 4*tmp_51)*(tmp_50*((u4_lU1)*(u4_lU1)) - tmp_57*(tmp_45 + tmp_47));
  ...
  const double tmp_87 = -tmp_1*vet_faceU0*(2*tmp_79 + 2*tmp_81) + 2*tmp_84*u4_rU0*u4_rU1;
  const double tmp_88 = (1.0/2.0)*tmp_86*tmp_87;
  const double tmp_90 = (4*tmp_83 + 4*tmp_85)*(-tmp_57*(tmp_79 + tmp_81) + tmp_84*((u4_rU1)*(u4_rU1)));
  ...
  conservative_fluxes->cmin_dirn1 = -1.0/4.0*tmp_103 - 1.0/4.0*tmp_106 + (1.0/4.0)*tmp_107 - 1.0/4.0*tmp_68 - 1.0/4.0*tmp_98 + (1.0/2.0)*fabs((1.0/2.0)*tmp_103 + (1.0/2.0)*tmp_106 - 1.0/2.0*tmp_107 + (1.0/2.0)*tmp_68 + (1.0/2.0)*tmp_98);
  conservative_fluxes->cmax_dirn1 = (1.0/4.0)*tmp_108 + (1.0/4.0)*tmp_109 + (1.0/4.0)*tmp_110 + (1.0/4.0)*tmp_111 + (1.0/4.0)*tmp_112 + (1.0/2.0)*fabs((1.0/2.0)*tmp_108 + (1.0/2.0)*tmp_109 + (1.0/2.0)*tmp_110 + (1.0/2.0)*tmp_111 + (1.0/2.0)*tmp_112);
}
}
